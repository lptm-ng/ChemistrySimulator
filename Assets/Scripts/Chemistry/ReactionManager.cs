...
public class ReactionManager : MonoBehaviour
{
    ...
    public void TriggerTollensReaction(ChemicalContainer container, List<string> contentsChemicals)
    {
        ...
        if (hasSilver)
        {
            ...
            else if (ammoniaCount >= 2)
            {
                ...
                TriggerReaction(container, "Tollensreagenz",
                    "Ag<sup>+</sup> + 2 NH<sub>3</sub> → [Ag(NH<sub>3</sub>)<sub>2</sub>]<sup>+</sup>"); // klare LÖsung
            }
        }
        ...
        if (hasSilver && ammoniaCount >= 2 && contentsChemicals.Contains("Tartrat"))
        {
            if (container.isHot)
            {
                TriggerReaction(container, "Silberspiegel",
                    "C<sub>4</sub>H<sub>4</sub>O<sub>6</sub><sup>2-</sup> + 10 [Ag(NH<sub>3</sub>)<sub>2</sub>]<sup>+</sup> + 8 OH<sup>-</sup> → 4 CO<sub>2</sub> + 10 Ag + 20 NH<sub>3</sub> + 6 H<sub>2</sub>O (elementares Silber)");
            }
        }
    }
    ...
    private Color GetFlameColor(ChemicalData chemical)
    {
        ...
        return
            Color.blue; // nicht-leuchtende Flamme --> Flammenfärbung macht man nicht mit der leuchtenden Flamme aka. orange/rot
    }
}